...
router.get("/registration", (req, res) => {
    studentDB.find().countDocuments((err, count) => {
...
// Search The students details from navbar
// Check the student's details from the Class details
...
router.delete("/delete", (req, res) => {
    ...
    nurserySub.findOneAndDelete({students_id:students_id}).then(score =>{
        if(score){
            ...
            res.render("student_info",{student:false,success_msg});
        }
    }).catch(err => (err));
    studentDB.findByIdAndDelete(id).then(student => {
        if (student) {
            const success_msg = " STUDENT'S DETAILS AND SCORED HAVE BEEN DELETED ";
            res.render("student_info", {
                student: false,
...
router.post("/edit", (req, res) => {
    const id = req.body.id;
    studentDB.findById(id).then(student => {
        if (student) {
            res.render("studentInfoEdit", {
                student
            });
        }else{
            res.send("<h1 style='text-align:center;font-size:40px'> PROBLEM FOUND ON HANDLING EDITING BUTTON </h1>");
        }
    }).catch(err => console.log(err));
});
...
router.post("/admission_date", (req, res) => {
    const id = req.body.id;
    const success_msg = "Updated";
    studentDB.updateOne({
        _id: id
...
router.post("/father_name", (req, res) => {
    const {
        id,
        father
    } = req.body;
...
router.post("/mother_name", (req, res) => {
    const id = req.body.id;
    const success_msg = "Updated";
    studentDB.updateOne({
        _id: id
...
router.post("/dateOfBirth", (req, res) => {
    const id = req.body.id;
    const success_msg = "Updated";
    studentDB.updateOne({
        _id: id
...
router.post("/contact_number", (req, res) => {
    const id = req.body.id;
    const success_msg = "Updated";
    studentDB.updateOne({
        _id: id
...
router.post("/contact_rel", (req, res) => {
    const id = req.body.id;
    const success_msg = "Updated";
    studentDB.updateOne({
        _id: id
...
//-------------- Resuslt Entry Modal --------------//
router.post('/resultEntry', (req, res) => {
    const id = req.body.id;
...
router.post("/nursery",(req,res)=>{
    const {bangla_firstSem,bangla_secondSem,bangla_thirdSem,english_firstSem,english_secondSem,english_thirdSem,math_firstSem,math_secondSem,math_thirdSem,religion_firstSem,religion_secondSem,religion_thirdSem,general_knowledge_firstSem,general_knowledge_secondSem,general_knowledge_thirdSem,drawing_firstSem, drawing_secondSem,drawing_thirdSem,students_id}=req.body;

    nurserySub.findOne({students_id:students_id},(err,score) =>{
        studentDB.findOne({students_id:students_id},(err,student) =>{
            if(student && !score){
                ...
                var totalFirstSem,average,fnumber;
                ...
                var totalSecondSem,secondSemAverage,Snumber;
                ...
                //  Grading detection and showing the grade point
...
router.post("/nurseryResultEdit", (req,res)=>{
    const id = req.body.id;
    nurserySub.findOne({students_id:id},(err, score)=>{
        studentDB.findOne({students_id:id},(err, student)=>{
            if(score && student){
                ...
                res.send("<h1 style='text-align:center;position:absolute;top:30vh;left:40vw'> SCORE DID NOT FOUND </h1>");
            }
        });
    });
});
...
// POST ROUTES UPDATING THE SCORES
// FIRST SEMESTER'S SCORES UPDATING ROUTE
router.post("/nurseryFirstSemEdit",(req,res)=>{
    const {id,bangla_firstSem,english_firstSem,math_firstSem,religion_firstSem,drawing_firstSem,general_knowledge_firstSem} = req.body;
    
    nurserySub.updateOne({students_id:id},{$set:{"bangla.firstSem":bangla_firstSem,"english.firstSem":english_firstSem,"math.firstSem":math_firstSem,"religion.firstSem":religion_firstSem,"drawing.firstSem":drawing_firstSem,"general_knowledge.firstSem":general_knowledge_firstSem}}).then(score =>{
            studentDB.findOne({students_id:id}).then(student =>{
                nurserySub.findOne({students_id:id},(err,score)=>{
                    res.render("nurseryResultEdit",{student,score});
                });
            }).catch(err => res.send("<h1 style='text-align:center;position:absolute;top:30vh;left:30vw'> SOMETHING BAD HAPPEND </h1>"));
    }).catch(err => res.send("<h1 style='text-align:center;position:absolute;top:30vh;left:30vw'> 404 ERROR ! </h1>"));
});


// SECOND SEMESTER'S SCORES UPDATE ROUTE
router.post("/nurserySecondSemEdit",(req,res)=>{
    const {id,bangla_secondSem,english_secondSem,math_secondSem,religion_secondSem,drawing_secondSem,general_knowledge_secondSem} = req.body;

    nurserySub.updateOne({students_id: id},{$set:{"bangla.secondSem":bangla_secondSem,"english.secondSem":english_secondSem,"math.secondSem":math_secondSem,"religion.secondSem":religion_secondSem,"drawing.secondSem":drawing_secondSem,"general_knowledge.secondSem":general_knowledge_secondSem}}).then(score =>{
            studentDB.findOne({students_id:id}).then(student =>{
                nurserySub.findOne({students_id:id},(err,score)=>{
                    res.render("nurseryResultEdit",{student,score});
                });
...
router.post("/nurseryThirdSemEdit",(req,res)=>{
    const {id,bangla_thirdSem,english_thirdSem,math_thirdSem,religion_thirdSem,drawing_thirdSem,general_knowledge_thirdSem} = req.body;

    nurserySub.updateOne({students_id: id},{$set:{"bangla.thirdSem":bangla_thirdSem,"english.thirdSem":english_thirdSem,"math.thirdSem":math_thirdSem,"religion.thirdSem":religion_thirdSem,"drawing.thirdSem":drawing_thirdSem,"general_knowledge.thirdSem":general_knowledge_thirdSem}}).then(score =>{
            studentDB.findOne({students_id:id}).then(student =>{
                nurserySub.findOne({students_id:id},(err,score)=>{
                    res.render("nurseryResultEdit",{student,score});
                });
